...
PPGraphics_32bpp_generic::PPGraphics_32bpp_generic(pp_int32 w, pp_int32 h, pp_int32 p, void* buff) :
	PPGraphicsAbstract(w, h, p, buff),
	bitPosR(0), bitPosG(8), bitPosB(16)
{
}

void PPGraphics_32bpp_generic::setPixel(pp_int32 x, pp_int32 y)
{
		...
		x >= currentClipRect.x1 && x < currentClipRect.x2)
	{
		...
		*(pp_uint32*)buff = (currentColor.r << bitPosR) + 
			(currentColor.g << bitPosG) +
			(currentColor.b << bitPosB);		
	}	
}

void PPGraphics_32bpp_generic::setPixel(pp_int32 x, pp_int32 y, const PPColor& color)
{
	if (y >= currentClipRect.y1 && y < currentClipRect.y2 &&
		x >= currentClipRect.x1 && x < currentClipRect.x2)
	{
		pp_uint8* buff = (pp_uint8*)buffer+pitch*y+x*BPP;
		*(pp_uint32*)buff = (color.r << bitPosR) + 
			(color.g << bitPosG) +
			(color.b << bitPosB);		
	}	
}

void PPGraphics_32bpp_generic::fill(PPRect rect)
{
	...
	pp_uint32 rgb1 = (currentColor.r << bitPosR) + 
		(currentColor.g << bitPosG) +
		(currentColor.b << bitPosB);
...
void PPGraphics_32bpp_generic::fill()
{
	fill(currentClipRect);
}

void PPGraphics_32bpp_generic::drawHLine(pp_int32 x1, pp_int32 x2, pp_int32 y)
{
...
void PPGraphics_32bpp_generic::drawVLine(pp_int32 y1, pp_int32 y2, pp_int32 x)
{
...
void PPGraphics_32bpp_generic::drawLine(pp_int32 x1, pp_int32 y1, pp_int32 x2, pp_int32 y2)
{
	__PPGRAPHICSLINETEMPLATE
}

void PPGraphics_32bpp_generic::drawAntialiasedLine(pp_int32 x1, pp_int32 y1, pp_int32 x2, pp_int32 y2)
{
	__PPGRAPHICSAALINETEMPLATE	
}

void PPGraphics_32bpp_generic::blit(SimpleBitmap& bitmap, PPPoint p)
{
	pp_uint8* src = (pp_uint8*)bitmap.buffer;

	const pp_uint32 bitPosR = this->bitPosR;
	const pp_uint32 bitPosG = this->bitPosG;
	const pp_uint32 bitPosB = this->bitPosB;
	...
	for (int y = 0; y < bitmap.height; y++)
		for (int x = 0; x < bitmap.width; x++)
		{
			...
			if (src[0]||src[1]||src[2])
			{
				...
				*(pp_uint32*)buff = ((pp_uint32)src[0] << bitPosR) + 
...
void PPGraphics_32bpp_generic::blit(const pp_uint8* src, const PPPoint& p, const PPSize& size, pp_uint32 pitch, pp_uint32 bpp, pp_int32 intensity/* = 256*/)
{
	...
	if (intensity != 256)
	{
		for (pp_int32 y = 0; y < h; y++)
		{
			...
			{	
				*(pp_uint32*)dst = (((src[0]*intensity)>>8) << bitPosR) + 
					(((src[1]*intensity)>>8) << bitPosG) +
					(((src[2]*intensity)>>8) << bitPosB);		
				dst+=BPP;
				src+=bytesPerPixel;
			}
	...
	{	
		for (pp_int32 y = 0; y < h; y++)
		{
			for (pp_int32 x = 0; x < w; x++)
			{	
				*(pp_uint32*)dst = ((pp_uint32)src[0] << bitPosR) + 
					((pp_uint32)src[1] << bitPosG) +
					((pp_uint32)src[2] << bitPosB);		
				dst+=BPP;
				src+=bytesPerPixel;
...
void PPGraphics_32bpp_generic::drawChar(pp_uint8 chr, pp_int32 x, pp_int32 y, bool underlined/* = false*/)
{
		...
		y>= currentClipRect.y1 && y + charHeight < currentClipRect.y2)
	{
		...
		for (pp_uint32 i = 0; i < charHeight; i++)
		{
			pp_uint32 xChr = yChr;
			for (pp_uint32 j = 0; j < charWidth; j++)
			{
...
void PPGraphics_32bpp_generic::drawString(const char* str, pp_int32 x, pp_int32 y, bool underlined/* = false*/)
{
	if (currentFont == NULL)
		return;
...
void PPGraphics_32bpp_generic::drawStringVertical(const char* str, pp_int32 x, pp_int32 y, bool underlined/* = false*/)
{
	if (currentFont == NULL)
		return;
