...
PPGraphics_16BIT::PPGraphics_16BIT(pp_int32 w, pp_int32 h, pp_int32 p, void* buff) :
	PPGraphicsAbstract(w, h, p, buff)
{
}

void PPGraphics_16BIT::setPixel(pp_int32 x, pp_int32 y)
{
...
void PPGraphics_16BIT::setPixel(pp_int32 x, pp_int32 y, const PPColor& color)
{
	if (y >= currentClipRect.y1 && y < currentClipRect.y2 &&
		x >= currentClipRect.x1 && x < currentClipRect.x2)
	{
...
void PPGraphics_16BIT::fill(PPRect rect)
{
	...
	pp_uint32 col32 = (((pp_uint32)color16) << 16) + color16;
	...
	const pp_int16 color16 = this->color16;
	...
	for (pp_int32 y = 0; y < height; y++)
	{
		...
		if (len&1)
			*((pp_uint16*)(buff32+(len>>1))) = color16;
...
void PPGraphics_16BIT::drawHLine(pp_int32 x1, pp_int32 x2, pp_int32 y)
{
...
void PPGraphics_16BIT::drawVLine(pp_int32 y1, pp_int32 y2, pp_int32 x)
{
...
void PPGraphics_16BIT::drawLine(pp_int32 x1, pp_int32 y1, pp_int32 x2, pp_int32 y2)
{	
	__PPGRAPHICSLINETEMPLATE
}

void PPGraphics_16BIT::drawAntialiasedLine(pp_int32 x1, pp_int32 y1, pp_int32 x2, pp_int32 y2)
{
	__PPGRAPHICSAALINETEMPLATE
}

void PPGraphics_16BIT::blit(SimpleBitmap& bitmap, PPPoint p)
{
	...
	for (int y = 0; y < bitmap.height; y++)
		...
		{		
			if (src[0]||src[1]||src[2])
			{
				...
				*buff = (((pp_uint16)((src[0])>>3)<<11)+((pp_uint16)((src[1])>>2)<<5)+(pp_uint16)((src[2])>>3));
			}
...
void PPGraphics_16BIT::blit(const pp_uint8* src, const PPPoint& p, const PPSize& size, pp_uint32 pitch, pp_uint32 bpp, pp_int32 intensity/* = 256*/)
{
	...
	if (intensity == 256)
	{
		...
		for (pp_int32 y = 0; y < h; y++)
		{
			...
			{	
				*dst = (((pp_uint16)((src[0])>>3)<<11)+((pp_uint16)((src[1])>>2)<<5)+(pp_uint16)((src[2])>>3));
				dst++;
				src+=bytesPerPixel;
			}
	...
	else
	{
		...
		for (pp_int32 y = 0; y < h; y++)
		{
			for (pp_int32 x = 0; x < w; x++)
			{	
				*dst = (((pp_uint16)((src[0]*intensity/256)>>3)<<11)+((pp_uint16)((src[1]*intensity/256)>>2)<<5)+(pp_uint16)((src[2]*intensity/256)>>3));
				dst++;
				src+=bytesPerPixel;
			}
			dst+=this->pitch/BPP - w;
...
void PPGraphics_16BIT::drawChar(pp_uint8 chr, pp_int32 x, pp_int32 y, bool underlined)
{
...
void PPGraphics_16BIT::drawString(const char* str, pp_int32 x, pp_int32 y, bool underlined/* = false*/)
{
	if (currentFont == NULL)
		return;
...
void PPGraphics_16BIT::drawStringVertical(const char* str, pp_int32 x, pp_int32 y, bool underlined/* = false*/)
{
	if (currentFont == NULL)
		return;
